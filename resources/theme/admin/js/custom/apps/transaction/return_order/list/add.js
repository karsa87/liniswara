...
// Class definition
var KTAppReturnOrderSaveProduct = function () {
    let quill = null;
    ...
    const initQuill = () => {
        ...
        const elements = [
            '#kt_ecommerce_add_return_order_notes',
        ];
        ...
        elements.forEach(element => {
            ...
            let orderDescription = document.querySelector('textarea[name="return_notes"]').value;
    ...
    const initConditionsSelect2 = () => {
        ...
        const allConditionTypes = document.querySelectorAll('[data-kt-ecommerce-catalog-add-order="return_order_option"]');
        ...
        $('#form-select-discount-type').on('change', function () {
            ...
            if (selected == 1) {
                $('input[name="return_discount_price"]').val('');
                $('#div-discount-percentage').show();
                $('#div-discount-price').hide();
            } else if (selected == 2) {
                $('input[name="return_discount_percentage]').val('');
                ...
                if ($(type).data('url')) {
                    $(type).select2({
                        ...
                        ajax: {
                            ...
                            dataType: 'json',
                            data: function (params) {
                                var excludeIds = $('.return_details_select_product').map((_,el) => el.value).get();
                    ...
                    $(type).on("change", function (e) {
                        ...
                        if (data.length > 0 && $(this).val() != undefined && $(this).val() != null && $(this).val() != '') {
                            ...
                            $(type).closest("tr").children()[3].querySelector('input.return_details_price').value = price;
                            ...
                            $(type).closest("tr").children()[4].querySelector('input.return_details_discount').value = discount;
                            $(type).closest("tr").children()[4].querySelector('input.return_details_discount_description').value = data.discount_description;
                            ...
                            $(this).closest("tr").find('input.return_detail_qty').removeAttr('disabled');
                            ...
                            $(type).closest("tr").children()[3].querySelector('input.return_details_price').value = 0;
                            ...
                            $(type).closest("tr").children()[4].querySelector('input.return_details_discount').value = 0;
                            $(type).closest("tr").children()[4].querySelector('input.return_details_discount_description').value = '';
                            ...
                            $(this).closest("tr").find('input.return_detail_qty').attr('disabled');
                        }
                    });
                } else {
                    $(type).select2({
    ...
    const initConditionsQuantity = () => {
        // // Tnit new repeating condition types
        $(".return_detail_qty").on('change', function (e) {
            let productEl = $(this).closest("tr").find('span.return_details_select_product');
            let dataSelected = $(productEl).data();

            let qty = $(this).closest("tr").find('input.return_detail_qty').val();
    ...
    const handleSubmit = () => {
        ...
        // Get elements
        const form = document.getElementById('kt_ecommerce_add_return_order_form');
        const submitButton = document.getElementById('kt_ecommerce_add_return_order_submit');
            ...
            if (validator) {
                validator.validate().then(function (status) {
                    if (status == 'Valid') {
                        ...
                        form.querySelector('textarea[name="return_notes"]').value = quill.root.innerHTML;
                        form.querySelector('textarea[name="return_notes"]').text = quill.root.innerHTML;

                        let param = new FormData(form);
                        // console.log(JSON.stringify(Object.fromEntries(param)));
                        let formSubmit = null;
                        if (param.get('return_id') != null && param.get('return_id') != undefined  && param.get('return_id') != '') {
                            param.append('_method', 'PUT');

                            formSubmit = axios.post(
                                submitButton.closest('form').getAttribute('action-update') + '/' + param.get('return_id'),
                                param,
                                {
                                    headers: {
    ...
    const initForm = () => {
        // Init flatpickr
        $('#kt_ecommerce_edit_return_order_date').flatpickr({
            altInput: true,
            altFormat: "d F, Y",
            dateFormat: "Y-m-d",
        });
        ...
        // Init flatpickr
        $('#kt_ecommerce_edit_return_order_paid_at').flatpickr({
            altInput: true,
            altFormat: "d F, Y",
            dateFormat: "Y-m-d",
        });
...
KTUtil.onDOMContentLoaded(function () {
    KTAppReturnOrderSaveProduct.init();
});
