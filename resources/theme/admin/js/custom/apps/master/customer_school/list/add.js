...
// Class definition
var KTCustomerSchoolsAdd = function () {
    // Shared variables
    ...
    var initAddCustomerSchool = () => {
        $('[name="customer_school_id"]').select2({
            dropdownParent: $('#kt_modal_add_customer_school'),
            ajax: {
                url: $('[name="customer_school_id"]').data('url'),
                dataType: 'json',
            ...
            if (validator) {
                validator.validate().then(function (status) {
                    if (status == 'Valid') {
                        ...
                        let formSubmit = null;
                        if (param.get('customer_school_id') != null && param.get('customer_school_id') != undefined  && param.get('customer_school_id') != '') {
                            param.append('_method', 'PUT');

                            formSubmit = axios.post(
                                submitButton.closest('form').getAttribute('action-update') + '/' + param.get('customer_school_id'),
                                param
                            )
                        } else {
